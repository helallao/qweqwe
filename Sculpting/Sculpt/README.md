...
* [Expand Mask by Normals](#expand-mask-by-normals)
* [Mask Extract](#mask-extract)
* [Mask Slice](#mask-slice)
* [Mask Slice and Fill Holes](#mask-slice-and-fill-holes)
* [Mask Slice to New Object](#mask-slice-to-new-object)
* [Mask From Cavity](#mask-from-cavity)
* [Random Mask](#random-mask)

# [Face Set İşlemleri](#face-set-i̇şlemleri-1)
* [Face Set from Masked](#face-set-from-masked)
* [Face Set from Visible](#face-set-from-visible)
* [Face Set from Edit Mode Selection](#face-set-from-edit-mode-selection)
* [Initialize Face Sets](#initialize-face-sets)
* [Grow Face Set](#grow-face-set)
* [Shrink Face Set](#shrink-face-set)
* [Expand Face Set by Topology](#expand-face-set-by-topology)
* [Expand Active Face Set](#expand-active-face-set)
* [Extract Face Set](#extract-face-set)
...
Vertex Paint modundaki [Draw](../Vertex%20Paint#draw) brush'ı ile aynıdır. Ek özelliklere de sahiptir.
...
* #### Color
Brush'ın rengini ayarlamanıza yarar. "Color" ve "Secondary Color" şeklinde iki renk seçebilirsiniz. Çizim yaparken geçici olarak 2. rengi kullanmak için çizime başlamadan önce "Ctrl" kısayoluna basılı tutmanız yeterlidir. İki rengin yerini değiştirmek için ise yandaki "Swap Colors" butonunu veya "X" kısayolunu kullanabilirsiniz ([Kısayollar](#kısayollar-1) bölümünden "Genel Kısayollar" a bakın). Onun yanındaki "Use Unified Color" ayarını açarak da renk değerini bütün fırçalara uygulayabilirsiniz. Ayrıca mouse'unuzun altındaki vertex'in rengini brush'ınızın rengi olarak ayarlamak için de "Shift + X" kısayolunu kullanabilirsiniz ([Kısayollar](#kısayollar-1) bölümünden "Genel Kısayollar" a bakın).
...
* #### Angle
Texture'un açısını değiştirmenize yani texture'u döndürmenize yarar. "Ctrl + F" kısayolu ile ayarlayabilirsiniz ([Kısayollar](#kısayollar-1) bölümünden "Genel Kısayollar" a bakın).
...
Tıkladığınız noktadan mouse'unuzu hareket ettirdikçe çevreye doğru geometriyi takip ederek yayılan bir maske oluşturma aracıdır. Kısayolu "Shift + A" dır ([Kısayollar](#kısayollar-1) bölümünden "Maske Kısayolları" na bakın). Ayrıca bu aracı ister toolbar (araç çubuğu) menüsünden, ister kısayola basarak kullanın, bu araç mouse'unuzun mesh üzerindeki en son konumunu baz alarak çalışır, yani mesela mouse'unuzu mesh'in bir köşesinin üzerine getirin sonra da mouse'unuzu kenara çekin ve kısayola tıklayın, aracın mouse'unuzun mesh üzerinde bulunduğu son konumu baz alarak çalıştığını görebilirsiniz. Ayrıca bu aracı kullanırken mouse'unuzun mesh üzerinde bulunduğu son konumun üzerinde bir nokta çıkar, böylelikle aracın nereyi baz alarak çalıştığını anlayabilirsiniz. Ayrıca bu aracı kullanırken değiştirebileceğiniz ayarlar vardır, bu ayarlara kısayollar aracılığı ile ulaşabilirsiniz. Aşağıdaki listeden kısayolları ve yaptığı işlemleri bulabilirsiniz.

Kısayol | Açıklama
:---: | :---:
Boşluk (Space) | Bu kısayola basılı tuttuğunuz sürece işleme başladığınız noktayı yani orta noktayı hareket ettirebilirsiniz.
F | İşlemi tersine çevirir yani maske oluşturmaya değil maske silmeye yarar.
1 | Geodesic Falloff moduna geçer. Geodesic Falloff modu daire şekline benzeyen ama mükemmel daire olmayan yani geometriye göre şekli değişebilen bir falloff modudur.
2 | Topology Falloff moduna geçer. Topology Falloff modu kenarları kullanan bir falloff modudur.
3 | Diagonals Falloff moduna geçer. Diagonals Falloff modu kenarları ve köşegenleri kullanan bir falloff modudur.
...
Tıkladığınız noktadan mouse'unuzu hareket ettirdikçe çevreye doğru geometriyi takip ederek yayılan bir face set oluşturma aracıdır. Kısayolu "Shift + W" dır ([Kısayollar](#kısayollar-1) bölümünden "Face Set Kısayolları" na bakın). Ayrıca bu aracı ister toolbar (araç çubuğu) menüsünden, ister kısayola basarak kullanın, bu araç mouse'unuzun mesh üzerindeki en son konumunu baz alarak çalışır, yani mesela mouse'unuzu mesh'in bir köşesinin üzerine getirin sonra da mouse'unuzu kenara çekin ve kısayola tıklayın, aracın mouse'unuzun mesh üzerinde bulunduğu son konumu baz alarak çalıştığını görebilirsiniz. Ayrıca bu aracı kullanırken mouse'unuzun mesh üzerinde bulunduğu son konumun üzerinde bir nokta çıkar, böylelikle aracın nereyi baz alarak çalıştığını anlayabilirsiniz. Ayrıca bu aracı kullanırken [Expand Mask by Topology](#expand-mask-by-topology) aracının sahip olduğu kısayolların çoğunu kullanabilirsiniz ama bu ayarların bazıları düzgün bir şekilde çalışmayabilir. Ayrıca "E" kısayolu oluşturulan yeni face set'in diğer face set'lerin üzerine mi geleceğini yoksa diğer face set'leri koruması mı gerektiğini belirtir.
...
[Expand Face Set by Topology](#expand-face-set-by-topology) ile aynıdır, tek farkı yeni face set oluşturmak yerine mouse'unuzun altındaki face set'i genişletmesidir. Kısayolu "Shift + Alt + W" dır ([Kısayollar](#kısayollar-1) bölümünden "Face Set Kısayolları" na bakın). Ayrıca bu aracı kullanırken [Expand Mask by Topology](#expand-mask-by-topology) aracının sahip olduğu kısayolların çoğunu kullanabilirsiniz ama bu ayarların bazıları düzgün bir şekilde çalışmayabilir. Ayrıca "E" kısayolu oluşturulan yeni face set'in diğer face set'lerin üzerine mi geleceğini yoksa diğer face set'leri koruması mı gerektiğini belirtir.
